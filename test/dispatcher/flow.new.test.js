...
describe('Dispatcher#flow (NEW)', function() {
  ...
  describe('return from federation with default processing', function() {
    var dispatcher = new Dispatcher()
      , request, response, err;
    ...
    before(function(done) {
      ...
      chai.express.handler(dispatcher.flow([ handler ], [ postHandler ]))
        .req(function(req) {
          request = req;
          request.method = 'GET';
          request.url = '/oauth2/redirect?code=SplxlOBeZQQYbYS6WxSbIA&state=af0ifjsldkj';
  ...
  describe('return from federation with default processing and option', function() {
    var dispatcher = new Dispatcher()
      , request, response, err;
    ...
    before(function(done) {
      ...
      chai.express.handler(dispatcher.flow([ handler ], [ postHandler ], { getHandle: function(req) { return 'oauth_' + req.query.oauth_token; } }))
        .req(function(req) {
          request = req;
          request.method = 'GET';
          request.url = '/oauth2/callback?oauth_token=XXXXXXXX&oauth_verifier=YYYYYYYY';
