...
describe('Dispatcher#flow', function() {
  ...
  }); // resuming with error parent state referenced by body param through unsynthesized state which resumes by rendering
  
  describe('resuming with error parent state referenced by body param through unsynthesized state which keeps state and finishes by rendering', function() {
    var hc = 1;
    var dispatcher = new Dispatcher({ genh: function() { return 'H' + hc++; } })
      , request, response, layout, err;
  ...
  }); // resuming with error parent state referenced by body param through unsynthesized state which keeps state and finishes by rendering
