...
describe('IncomingMessage#pushState', function() {
  ...
  it('should redirect after saving pushed state that redirects to referrer header', function(done) {
    var store = new SessionStore();
    sinon.spy(store, 'get');
    sinon.spy(store, 'set');
    sinon.spy(store, 'destroy');
    ...
    chai.express.use([ state({ store: store, genh: function() { return 'xyz' } }), handler ])
      ...
      .listen();
  }); // should redirect after saving pushed state that redirects to referrer header
