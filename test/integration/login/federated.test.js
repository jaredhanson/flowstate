...
describe('GET /login/federated', function() {
  ...
  it('should redirect with state which then returns to relative location', function(done) {
    var store = new SessionStore();
    sinon.spy(store, 'get');
    sinon.spy(store, 'set');
    sinon.spy(store, 'destroy');
    ...
    chai.express.use([ state({ store: store, genh: function() { return 'xyz' } }), handler ])
      .request(function(req, res) {
        req.connection = { encrypted: true };
        req.method = 'GET';
        req.url = '/login/federated?provider=https%3A%2F%2Fserver.example.com&return_to=/welcome';
        req.headers = {
          'host': 'client.example.com',
          'referer': 'https://client.example.com/login'
        };
        req.query = { provider: 'https://server.example.com', return_to: '/welcome' };
        req.session = {};
      })
      .finish(function() {
        expect(store.get).to.have.callCount(0);
      ...
      .listen();
  }); // should redirect with state which then returns to relative location
  ...
  // TODO: review this
  // FIXME: this is failing because an extraneous return_to is being added, which it shouldn't
  it.skip('should initialize state with state query parameter and redirect with saved pushed state', function(done) {
    var store = new SessionStore();
    sinon.spy(store, 'get');
    sinon.spy(store, 'set');
    sinon.spy(store, 'destroy');
  ...
  // TODO: review this
  // FIXME: this is failing because an extraneous return_to is being added, which it shouldn't
  it.skip('should initialize state with state query parameter and redirect with saved pushed state using handle option', function(done) {
    var store = new SessionStore();
    sinon.spy(store, 'get');
    sinon.spy(store, 'set');
    sinon.spy(store, 'destroy');
