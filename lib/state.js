...
defineMethod(State.prototype, 'push', function push(data) {
  if (this.returnTo) {
    data.returnTo = this.returnTo;
  } else if (this.resumeState) {
    data.resumeState = this.resumeState;
  }
  ...
  // TODO: This parent property can be removed
  this.parent = this;
  var state = new State(this._req, data);
  state.touch();
  this._req._stateStack.push(state);
  this._req.state = state;
});
