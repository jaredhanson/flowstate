  ...
  return function state(req, res, next) {
    ...
    res.resumeState = function(yields, cb) {
      ...
      if (req.state.returnTo) {
        ...
        // TODO: Should be dead code.  don't get from store when resuming state
        var self = this;
        store.get(req, req.state.state, function(err, data) {
          ...
          if (!data) {
            ...
            // TODO: Handle the case where there is a return to here.
            return;
          }
    ...
    swizzleRedirect(res, function(url, preventReturnTo, cb) {
      ...
      commit2(function(err, nstate) {
        ...
        console.log('NSTATE IS');
        console.log(nstate);
        console.log(nstate.handle);
        console.log(nstate.isComplete());
        
        var returnTo = nstate.returnTo;
        var state = nstate.state;
        */
      
        /*
        if (!nstate.isComplete()) { // handles push from external condition?
          console.log('NOT COMLETE');
          
          var l = uri.parse(url, true);
          delete l.search;
          l.query.return_to = nstate.location;
          l.query.state = nstate.handle;
        ...
        */
        ...
        if (!nstate.handle) {
          ...
          console.log('X2')
          //console.log(nstate);
          console.log(nstate.isContinue());
          
          var l = uri.parse(url, true);
          delete l.search;
          if (nstate.isContinue()) { l.query.return_to = nstate.location; }
          //if (nstate.isExternal()) { l.query.return_to = returnTo; }
          //l.query.return_to = nstate.location;
          l.query.state = nstate.handle;
          return cb(null, uri.format(l));
        }
      ...
      */
      ...
      commit2(function(err, nstate) {
        ...
        if (!req.state.handle) {
          ...
          // TODO: improve the semantics of this test here.
          if (req.state.isContinue()) {
            l.query.return_to = req.state.location;
          }
          if (req.state.isModified()) {
            // TODO: This condition can, I beleive, be removed as it adds a redundant parameter.
            // tests need to be reviewed and updated, if so
            l.query.return_to = req.state.location;
          }
    ...
    swizzleRender(res, function(cb) {
      ...
      commit2(function(err, nstate) {
        ...
        if (!req.state.handle) {
          // Request processing didn't introduce any state that needed to be
          // preserved for subsequent requests.
          //
          // TODO: clarify docs
          // The state to eventually resume was initialized using the redirect
          // URL and optional state parameters.  Request processing didn't introduce
          // any additional state to capture, so none was serialized to the state
          // store.  The state to eventually resume can be carried forward by
          // preserving the parameters.
          //
          // The state was loaded from its handle and completed.  It captured a
          // state to eventually resume
