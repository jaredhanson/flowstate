  ...
  return function state(req, res, next) {
    ...
    swizzleRender(res, function(cb) {
      ...
      commit2(function(err, nstate) {
        ...
        if (nstate.isNew()) {
          ...
          if (nstate.returnTo) { res.locals.returnTo = nstate.returnTo; }
          if (nstate.state) { res.locals.state = nstate.state; }
        } else if (nstate.isComplete()) {
          // The state was loaded from its handle and completed.  It captured a
          // state to eventually resume
          if (nstate.returnTo) { res.locals.returnTo = nstate.returnTo; }
          if (nstate.state) { res.locals.state = nstate.state; }
        } else {
          res.locals.state = nstate.handle;
        }
        cb();
      });
    ...
    function commit2(cb) {
      ...
      function iter(err, nstate) {
        ...
        var state = stack[i++];
        if (!state) { return cb(null, nstate); }
