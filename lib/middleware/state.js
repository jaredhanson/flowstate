...
module.exports = function(options) {
  ...
  return function state(req, res, next) {
    ...
    //
    // The latter case is common with federated protocols, such as OAuth 2.0, in
    // which which clients make requests that initiate stateful transactions
    // within the app making use of this middleware.  These requests may have a
    // `state` parameter, but its value is meant to be included when the browser
    // is redirected back to the client so the client can maintain its state.
    var h = getHandle(req);
    ...
    store.load(req, h, function(err, state) {
      ...
      if (state.location !== url) {
        // The loaded state is not intended for the endpoint that was requested.
