...
module.exports = function(dispatcher, store, name, _rest) {
  ...
  function initialize(req, res, next) {
    ...
    onRender(res, function autoState(cb) {
      debug('rendering %O (new: %o, changed: %o)', req.state, req.state.isNew(), req.state.isChanged());
      ...
      // TODO else if if (req.state.isChanged()) {
    });
    ...
    onEnd(res, function autoDestroy(cb) {
      ...
      console.log('new: ' + req.state.isNew());
      console.log('changed: ' + req.state.isChanged());
      console.log('complete: ' + req.state.isComplete());
      console.log('saved: ' + req.state.isSaved());
