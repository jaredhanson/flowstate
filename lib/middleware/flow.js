...
module.exports = function(dispatcher, store, name, _rest) {
  ...
  function initialize(req, res, next) {
    ...
    swizzleRedirect(res, function commitState(url, cb) {
      debug('redirect %s, %O (handle: %s, modified: %o, saved: %o, complete: %o, keep: %o)', url, req.state,
        req.state.handle, req.state.isModified(), req.state.isSaved(), req.state.isComplete(), req.state.isKeeped());
      ...
      if (req.state.isExternal()) {
        ...
        if (!req.state.parent && (req.query && req.query.state) || (req.body && req.body.state)) {
          proceed((req.query && req.query.state) || (req.body && req.body.state));
          return
        }
    ...
    swizzleRender(res, function commitState(cb) {
      debug('render %O (handle: %s, modified: %o, saved: %o, complete: %o, keep: %o)', req.state,
        req.state.handle, req.state.isModified(), req.state.isSaved(), req.state.isComplete(), req.state.isKeeped());
