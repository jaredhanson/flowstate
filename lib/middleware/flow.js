...
module.exports = function(dispatcher, store, name, _rest) {
  ...
  var state = dispatcher._states[aname];
  
  if (options.external) {
    fns.unshift(initialize);
  } else {
    fns.unshift(loadState(store, aname, options));
    fns.unshift(initialize);
  }
  
  if (state) {
    // TODO: test cases for this
    //fns.push.apply(fns, state.prompt);
    fns.push(require('./prompt')(state, actionMethods));
  }
  ...
  function initialize(req, res, next) {
    ...
    req.state = new State(req, { name: aname || req.originalUrl || req.url }, undefined, options.external);
    ...
    res.prompt = function prompt(name, options) {
      ...
      if (!name) {
        // reprompt existing state.
        var state = dispatcher._states[req.state.name]
        if (state && state.prompt) {
          dispatch(state.prompt)(null, req, res, function(err) {
            // TODO, error here, because no prompt is available
          ...
          // TODO: Forcibly error, developer failuer.  Skip remaining middleware
        }
        return;
      }
