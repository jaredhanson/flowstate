  ...
  function initialize(req, res, next) {
    ...
    swizzleRedirect(res, function commitState(url, cb) {
      ...
      if (req.state.isComplete()) {
        proceed();
      } else if (req.state.isSaved()) {
        // TODO: Test case for this.  What situation would this be?
        ...
        // TODO: carry existing state handle????
        proceed();
      }
    });
    ...
    swizzleRender(res, function commitState(cb) {
      ...
      if ((req.state.isNew() && req.state.isTouched()) || req.state.isModified()) {
        req.state.save(function(err) {
          // TODO: error handling
