...
Manager.prototype._through = function(through, err, req, res, next) {
  ...
  return this._dispatch(through, null, err, req, res, next);
}

Manager.prototype._dispatch = function(name, through, err, req, res, next) {
  ...
  if (tname && tname !== req.state.name) {
    ...
    return this._dispatch(tname, null, err, req, res, next);
  }
  ...
  if (req.yieldState) {
    ...
    this._transition(name, req.yieldState.name, err, req, res, cont);
  } else {
    cont(err);
  }
}
...
Manager.prototype._complete = function(options, err, req, res, next) {
  ...
  function dispatch() {
    return self._dispatch(req.state.name, through, err, req, res, next);
  }
