...
Manager.prototype._complete = function(options, err, req, res, next) {
  ...
  if (req._state) {
    ...
    // The current state is complete and will be removed from any persistent
    // storage.  Once removed, the parent state (if any) will be resumed.
    //if (req.state.isNew() && !req.yieldState) {
    if (req.state.isNew() && !req.state.isSynthentic()) {
