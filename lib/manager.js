...
Manager.prototype._continue = function(err, req, res, next) {
  ...
  if (!state) { return next(new Error("Cannot find flow '" + name + "'")); }
  if (!state.resume) { return next(new Error("Cannot resume flow '" + name + "'")); }
  ...
  var arr = state.resume.concat([comp, compErr]);
  if (state.finish) {
    arr = arr.concat(state.finish)
  }
