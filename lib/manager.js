...
Manager.prototype.use = function(name, state) {
  ...
  this._states[name] = {
    spawn: state.spawn && flatten(Array.prototype.slice.call(state.spawn, 0)),
    start: null,
    resume: state.resume && flatten(Array.prototype.slice.call(state.resume, 0)),
    prompt: state.prompt && flatten(Array.prototype.slice.call(state.prompt, 0)),
...
Manager.prototype.goto = function(name, options, req, res, next) {
  ...
  if (!flow.spawn) { throw new Error("Unable to launch state '" + name + "'"); }
  
  if (options) {
    req.locals = options;
  }
  dispatch(flow.spawn)(null, req, res, next);
}
...
Manager.prototype._continue = function(err, req, res, next) {
  ...
  if (state.prompt) {
    ...
    //stack.push.apply(stack, state.prompt)
  }
  if (state.exit) {
